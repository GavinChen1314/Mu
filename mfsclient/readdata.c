...
//	worker_term_waiting = 0;
	...
	indhash = jemalloc(sizeof(inodedata*)*IDHASHSIZE);
	passert(indhash);
...
int read_data(void *vid, uint64_t offset, uint32_t *size, void **vrhead,struct iovec **iov,uint32_t *iovcnt) {
	...
	*iovcnt = 0;
	...
	if (ind->status==0 && ind->closing==0) {
		...
		if (ranges==NULL) {
			ranges = jemalloc(sizeof(uint64_t)*11);
			passert(ranges);
...
//				rl->rreq->waiting--;
	...
	if (ind->status==0 && ind->closing==0 && cnt>0) {
		...
		*iov = jemalloc(sizeof(struct iovec)*cnt);
		passert(*iov);
