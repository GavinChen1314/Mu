...
static inline xattr_cache_value* xattr_cache_value_alloc(void) {
	xattr_cache_value *v;
	v = jemalloc(sizeof(xattr_cache_value));
	passert(v);
	v->lcnt = 1;
	v->value = NULL;
	return v;
...
static inline void xattr_cache_new(uint32_t node,uint32_t uid,uint32_t gid,uint32_t nleng,const uint8_t *name,const uint8_t *value,uint32_t vleng,int status,int64_t utimestamp) {
	...
	xce = jemalloc(sizeof(xattr_cache_entry));
	passert(xce);
	xce->hash = hash;
	xce->node = node;
	xce->uid = uid;
	...
	if (vleng>0) {
		xce->value->value = jemalloc(vleng);
...
void xattr_cache_init(double timeout) {
	...
	lruhead = NULL;
	lrutail = &lruhead;
	hashtab = jemalloc(sizeof(xattr_cache_entry*)*HASHSIZE);
