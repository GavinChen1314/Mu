...
//		syslog(LOG_NOTICE,"receive thread for %s got request (cmd:%s)",nbdcp->nbddevice,nbd_cmd_str(cmd));
		if (cmd==NBD_CMD_WRITE || cmd==NBD_CMD_READ) {
			r = (nbdrequest*)jemalloc(offsetof(nbdrequest,data)+length);
		} else {
			r = (nbdrequest*)jemalloc(offsetof(nbdrequest,data));
		}
		passert(r);
		r->nbdcp = nbdcp;
...
char* linkname_generate(char *linkname,const char *masterhost,const char *masterport,const char *filename) {
	...
	if (linkname!=NULL) {
		...
		ln = jemalloc(fnl+NBD_LINK_PREFIX_LENG+1);
		passert(ln);
		...
		ln = jemalloc(mhl+mpl+fnl+3+1+NBD_LINK_PREFIX_LENG);
		passert(ln);
...
void nbd_handle_add_device(int sock,const uint8_t *buff,uint32_t leng) {
	...
	if (pleng==0) {
		...
		bdl = jemalloc(sizeof(bdlist));
		passert(bdl);
		bdl->nbdcp = jemalloc(sizeof(nbdcommon));
		passert(bdl->nbdcp);
