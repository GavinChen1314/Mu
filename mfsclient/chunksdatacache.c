...
static inline chunks_data_entry* chunks_new_entry(uint32_t inode,uint32_t chindx) {
	...
	if (ih==NULL) {
		ih = jemalloc(sizeof(chunks_inode_entry));
		ih->inode = inode;
		ih->data_head = NULL;
	...
	ca = jemalloc(sizeof(chunks_data_entry));
	ca->inode = inode;
	ca->chindx = chindx;
	ca->chunkid = 0;
	ca->version = 0;
...
void chunksdatacache_init(void) {
	...
	chunks_inode_hash = jemalloc(sizeof(chunks_inode_entry*)*CHUNKS_INODE_HASH_SIZE);
	passert(chunks_inode_hash);

	chunks_data_hash = jemalloc(sizeof(chunks_data_entry*)*CHUNKS_DATA_HASH_SIZE);
