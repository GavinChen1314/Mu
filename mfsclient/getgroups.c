...
// as comma separated list of gids at end of (single) line.
	...
	if (fd==NULL) {
		*gidtab = jemalloc(sizeof(uint32_t)*1);
		passert(*gidtab);
		(*gidtab)[0] = gid;
		return 1;
	}
	while (fgets(linebuff,4096,fd)) {
		if (strncmp(linebuff,"Groups:",7)==0) {
			...
			*gidtab = jemalloc(sizeof(uint32_t)*gcount);
			passert(*gidtab);
			(*gidtab)[0] = gid;
			n = 1;
			ptr = linebuff+7;
...
// the only problem ... only root can access this files for all processes !!!
	...
	if (gcount==n-7 && gcount>0) {
		...
		*gidtab = jemalloc(sizeof(uint32_t)*gids);
		passert(*gidtab);
		(*gidtab)[0] = gid;
		gids = 1;
		for (n=0 ; n<gcount ; n++) {
...
//			fprintf(stderr,"groups_get check node (%"PRIu32",%"PRIu32",%"PRIu32")\n",g->pid,g->uid,g->gid);
	...
	if (g) {
		...
		if (lockmode==1) { // emergency case
			...
			g->gidtab = jemalloc(sizeof(uint32_t));
			g->gidtab[0] = gid;
			g->gidcnt = 1;
			g->locked = 1;
		} else {
...
void groups_init(double _to,int dm) {
	...
	groups_hashtab = jemalloc(sizeof(groups*)*HASHSIZE);
