...
void sstats_activate(uint32_t inode) {
	...
	if (isc==NULL) {
		...
		isc = jemalloc(sizeof(inode_stats));
		passert(isc);
		isc->inode = inode;
...
void sstats_init(void) {
	uint32_t i;
	ishash = (inode_stats**)jemalloc(sizeof(inode_stats*)*HASHSIZE);
	passert(ishash);
	locktab = (pthread_mutex_t*)jemalloc(sizeof(pthread_mutex_t)*HASHSIZE);
	passert(locktab);
	for (i=0 ; i<HASHSIZE ; i++) {
		ishash[i] = NULL;
