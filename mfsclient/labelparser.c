...
static inline termval* expr_eval(node *a) {
	...
	if (a->op==SYM) {
		t1 = jemalloc(sizeof(termval));
		t1->cnt = 1;
		t1->labelmasks = jemalloc(sizeof(uint32_t));
	...
	if (a->op==OR || a->op==AND) {
		...
		t = jemalloc(sizeof(termval));
	}
	if (a->op==AND) {
		t->cnt = t1->cnt*t2->cnt;
		t->labelmasks = jemalloc(sizeof(uint32_t)*t->cnt);
		for (i=0 ; i<t1->cnt ; i++) {
			for (j=0 ; j<t2->cnt ; j++) {
