...
// init replication structure
	...
	r.created = 0;
	r.opened = 0;
	r.fds = jemalloc(sizeof(struct pollfd)*srccnt);
	passert(r.fds);
	r.repsources = jemalloc(sizeof(repsrc)*srccnt);
	passert(r.repsources);
	if (srccnt>1) {
		r.xorbuff = jemalloc(MFSBLOCKSIZE+4);
		passert(r.xorbuff);
	} else {
		r.xorbuff = NULL;
	}
