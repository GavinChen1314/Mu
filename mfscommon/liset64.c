...
int liset_new() {
	...
	if (settabffree>=settabsize) {
		if (settab==NULL) {
			settabsize = 10;
			settab = (liset*)jemalloc(sizeof(liset)*settabsize);
		} else {
			settabsize *= 2;
...
static inline uint32_t* liset_getbset(liset *lset,uint64_t value,int m) {
	...
	if (b0==NULL) {
		if (m==0) return NULL;
		b0 = lset->bittab = (uint32_t ****)jemalloc(sizeof(uint32_t ***)*65536);
		for (i=0 ; i<65536 ; i++) {
			b0[i]=NULL;
		}
	}
	...
	if (b1==NULL) {
		if (m==0) return NULL;
		b1 = b0[indx] = (uint32_t ***)jemalloc(sizeof(uint32_t **)*65536);
		for (i=0 ; i<65536 ; i++) {
			b1[i] = NULL;
		}
	}
	...
	if (b2==NULL) {
		if (m==0) return NULL;
		b2 = b1[indx] = (uint32_t **)jemalloc(sizeof(uint32_t*)*65536);
		for (i=0 ; i<65536 ; i++) {
			b2[i] = NULL;
		}
	}
	...
	if (b3==NULL) {
		if (m==0) return NULL;
		b3 = b2[indx] = (uint32_t *)jemalloc(sizeof(uint32_t)*(65536/32));
