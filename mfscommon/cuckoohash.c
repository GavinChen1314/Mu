...
//		printf("rehash\n");
		...
		for (i=ht->rehashpos>>HASHTAB_LOBITS ; i<ht->size>>HASHTAB_LOBITS ; i++) {
			ht->hashtab[i] = jemalloc(sizeof(hentry)*HASHTAB_LOSIZE);
...
void* chash_new() {
	...
	ht->hashtab = jemalloc(sizeof(hentry*)*HASHTAB_HISIZE);
	passert(ht->hashtab);
	ht->hashtab[0] = jemalloc(sizeof(hentry)*HASHTAB_LOSIZE);
