...
int charts_init (const uint32_t *calcs,const statdef *stats,const estatdef *estats,const char *filename,uint8_t mode) {
	...
	if (i>0 && calcdefscount>0) {
		calcdefs = (uint32_t*)jemalloc(sizeof(uint32_t)*i);
		passert(calcdefs);
		calcstartpos = (uint32_t**)jemalloc(sizeof(uint32_t*)*calcdefscount);
	...
	if (statdefscount>0) {
		statdefs = (statdef*)jemalloc(sizeof(statdef)*statdefscount);
		passert(statdefs);
	} else {
		statdefs = NULL;
	}
	...
	if (estatdefscount>0) {
		estatdefs = (estatdef*)jemalloc(sizeof(estatdef)*estatdefscount);
		passert(estatdefs);
	} else {
		estatdefs = NULL;
	}
	...
	if (statdefscount>0) {
		monotonic = (uint64_t*)jemalloc(sizeof(uint64_t)*statdefscount);
		passert(monotonic);
		series = (stat_record*)jemalloc(sizeof(stat_record)*statdefscount);
		passert(series);
		for (i=0 ; i<statdefscount ; i++) {
			monotonic[i] = 0;
			for (j=0 ; j<RANGES ; j++) {
				series[i][j] = jemalloc(MAXLENG*sizeof(uint64_t));
				passert(series[i][j]);
