...
int statsdump_parse (int32_t **stats,const char *opt) {
	...
	if (usedstats==NULL) {
		...
		if (statscnt>0) {
			usedstats = jemalloc((statscnt+7)&0xFFF8);
	...
	if (usedestats==NULL) {
		...
		if (estatscnt>0) {
			usedestats = jemalloc((estatscnt+7)&0xFFF8);
...
void statsdump_genpng(int32_t *stats,uint32_t rmask,const char *fname,uint32_t width,uint32_t height) {
	...
	for (i=0 ; stats[i]>=0 ; i++) {
		...
		data = jemalloc(pngsize);
		charts_get_png(data);
		snameleng = strlen(sname);
		if (dotpos>0) {
			ofname = jemalloc(fnameleng+1+snameleng+1);
			...
			ofname = jemalloc(fnameleng+1+snameleng+5);
...
void statsdump_gencsv(int32_t *stats,uint32_t rmask,char sep,uint8_t flags) {
	...
	data = jemalloc(sizeof(double*)*statscnt);
	for (i=0 ; i<statscnt ; i++) {
		data[i] = jemalloc(sizeof(double)*statsleng);
		if (stats[i]<100) {
			statid = statdefs[stats[i]].statid;
		} else {
			statid = estatdefs[stats[i]-100].statid;
