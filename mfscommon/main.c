...
void main_destruct_register_fname (void (*fun)(void),const char *fname) {
	deentry *aux=(deentry*)jemalloc(sizeof(deentry));
	passert(aux);
	aux->fun = fun;
...
void main_canexit_register_fname (int (*fun)(void),const char *fname) {
	ceentry *aux=(ceentry*)jemalloc(sizeof(ceentry));
	passert(aux);
	aux->fun = fun;
	aux->fname = strdup(fname);
	aux->next = cehead;
...
void main_wantexit_register_fname (void (*fun)(void),const char *fname) {
	weentry *aux=(weentry*)jemalloc(sizeof(weentry));
	passert(aux);
	aux->fun = fun;
	aux->fname = strdup(fname);
	aux->next = wehead;
...
void main_reload_register_fname (void (*fun)(void),const char *fname) {
	rlentry *aux=(rlentry*)jemalloc(sizeof(rlentry));
	passert(aux);
	aux->fun = fun;
	aux->fname = strdup(fname);
	aux->next = rlhead;
...
void main_info_register_fname (void (*fun)(void),const char *fname) {
	inentry *aux=(inentry*)jemalloc(sizeof(inentry));
	passert(aux);
	aux->fun = fun;
	aux->fname = strdup(fname);
	aux->next = inhead;
...
void main_keepalive_register_fname (void (*fun)(void),const char *fname) {
	kaentry *aux=(kaentry*)jemalloc(sizeof(kaentry));
	passert(aux);
	aux->fun = fun;
	aux->fname = strdup(fname);
	aux->next = kahead;
...
void main_poll_register_fname (void (*desc)(struct pollfd *,uint32_t *),void (*serve)(struct pollfd *),const char *dname,const char *sname) {
	pollentry *aux=(pollentry*)jemalloc(sizeof(pollentry));
	passert(aux);
	aux->desc = desc;
	aux->serve = serve;
...
void main_eachloop_register_fname (void (*fun)(void),const char *fname) {
	eloopentry *aux=(eloopentry*)jemalloc(sizeof(eloopentry));
	passert(aux);
	aux->fun = fun;
	aux->fname = strdup(fname);
	aux->next = eloophead;
...
void main_chld_register_fname (pid_t pid,void (*fun)(int),const char *fname) {
	chldentry *aux=(chldentry*)jemalloc(sizeof(chldentry));
	passert(aux);
	aux->pid = pid;
	aux->fun = fun;
	aux->fname = strdup(fname);
...
void* main_msectime_register_fname (uint32_t mseconds,uint32_t offset,void (*fun)(void),const char *fname) {
	...
	aux = (timeentry*)jemalloc(sizeof(timeentry));
	passert(aux);
