...
int sclass_load(bio *fd,uint8_t mver,int ignoreflag) {
	...
	databuff = jemalloc(3U*9U*4U*(uint32_t)orgroup);
	passert(databuff);
...
int sclass_init(void) {
	...
	for (i=0 ; i<MAXSCLASS ; i++) {
		...
		for (j=0 ; j<MAXLABELSCNT ; j++) {
			sclasstab[i].create_labelmasks[j]=jemalloc(MASKORGROUP*sizeof(uint32_t));
			passert(sclasstab[i].create_labelmasks[j]);
			sclasstab[i].keep_labelmasks[j]=jemalloc(MASKORGROUP*sizeof(uint32_t));
			passert(sclasstab[i].keep_labelmasks[j]);
			sclasstab[i].arch_labelmasks[j]=jemalloc(MASKORGROUP*sizeof(uint32_t));
