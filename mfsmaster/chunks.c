...
int32_t* do_perfect_match(uint32_t labelcnt,uint32_t servcnt,uint32_t **labelmasks,uint16_t *servers) {
	...
	if (labelcnt + servcnt > tablength || imatching==NULL || matching==NULL || augment==NULL || visited==NULL || stack==NULL) {
		...
		imatching = jemalloc(sizeof(int32_t)*tablength);
		passert(imatching);
		matching = jemalloc(sizeof(int32_t)*tablength);
		passert(matching);
		augment = jemalloc(sizeof(int32_t)*tablength);
		passert(augment);
		visited = jemalloc(sizeof(uint8_t)*tablength);
		passert(visited);
		stack = jemalloc(sizeof(int32_t)*tablength);
		passert(stack);
	}
	if (servcnt > stablength || sidval==NULL || sidpos==NULL || grnode==NULL || group==NULL) {
		...
		sidval = jemalloc(sizeof(uint32_t)*stablength);
		passert(sidval);
		sidpos = jemalloc(sizeof(int32_t)*stablength);
		passert(sidpos);
		grnode = jemalloc(sizeof(int32_t)*stablength);
		passert(grnode);
		group = jemalloc(sizeof(int32_t)*stablength);
		passert(group);
	}
...
#else
		chunkhashtab[i] = jemalloc(sizeof(chunk*)*HASHTAB_LOSIZE);
#endif
...
static inline void chunk_priority_queue_check(chunk *c,uint8_t checklabels) {
	...
	if (c==NULL) {
		if (servers==NULL && checklabels==0) {
			servers = jemalloc(sizeof(uint16_t)*MAXCSCOUNT);
			passert(servers);
		}
...
void chunk_addopchunk(uint16_t csid,uint64_t chunkid) {
	csopchunk *csop;
	csop = jemalloc(sizeof(csopchunk));
...
int chunk_change_file(uint64_t chunkid,uint8_t prevsclassid,uint8_t newsclassid) {
	...
	if (c->fcount==1) {
		...
		if (c->ftab==NULL) {
			...
			ftableng++;
			c->ftab = jemalloc(sizeof(uint32_t)*(ftableng+1));
			passert(c->ftab);
	...
	if (ochunkid==0) {	// new chunk
		if (mr==0) {
			...
			if (c->allvalidcopies>chosenleng) {
				...
				chosen = jemalloc(sizeof(void*)*chosenleng);
				passert(chosen);
			}
...
//	static uint16_t bservcount;
	...
	if (rcsids==NULL) {
		rcsids = jemalloc(sizeof(uint16_t)*MAXCSCOUNT);
		passert(rcsids);
	}
	if (dcsids==NULL) {
		dcsids = jemalloc(sizeof(uint16_t)*MAXCSCOUNT);
		passert(dcsids);
	}
//	if (bcsids==NULL) {
//		bcsids = jemalloc(sizeof(uint16_t)*MAXCSCOUNT);
//		passert(bcsids);
//	}
...
uint8_t chunk_labelset_can_be_fulfilled(uint8_t labelcnt,uint32_t **labelmasks) {
	...
	if (stdcsids==NULL) {
		stdcsids = jemalloc(sizeof(uint16_t)*MAXCSCOUNT);
		passert(stdcsids);
	}
	if (olcsids==NULL) {
		olcsids = jemalloc(sizeof(uint16_t)*MAXCSCOUNT);
		passert(olcsids);
	}
	if (allcsids==NULL) {
		allcsids = jemalloc(sizeof(uint16_t)*MAXCSCOUNT);
		passert(allcsids);
	}
...
//		HashSteps = 1+((HASHSIZE)/(LoopTimeMin*TICKSPERSECOND));
	...
	if (DangerMaxLeng != oldDangerMaxLeng) {
		for (j=0 ; j<DANGER_PRIORITIES ; j++) {
			...
			chunks_priority_queue[j] = (chunk**)jemalloc(sizeof(chunk*)*DangerMaxLeng);
...
//		chunkhash[i]=NULL;
//	}
	cstab = jemalloc(sizeof(csdata)*MAXCSCOUNT);
	passert(cstab);
	for (i=0 ; i<MAXCSCOUNT ; i++) {
		cstab[i].next = i+1;
		cstab[i].prev = i-1;
	...
	csfreetail = MAXCSCOUNT-1;
	csusedhead = MAXCSCOUNT;
	allchunkcounts = jemalloc(sizeof(uint32_t*)*MAXSCLASS*2);
	passert(allchunkcounts);
	regularchunkcounts = jemalloc(sizeof(uint32_t*)*MAXSCLASS*2);
	passert(regularchunkcounts);
	for (i=0 ; i<MAXSCLASS*2 ; i++) {
		allchunkcounts[i] = jemalloc(sizeof(uint32_t)*11);
		passert(allchunkcounts[i]);
		regularchunkcounts[i] = jemalloc(sizeof(uint32_t)*11);
