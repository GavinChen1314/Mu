...
repsrc* matocsserv_repsrc_jemalloc() {
	repsrc *r;
	if (repsrcfreehead) {
		r = repsrcfreehead;
		repsrcfreehead = r->next;
	} else {
		r = (repsrc*)jemalloc(sizeof(repsrc));
		passert(r);
	}
	return r;
}
...
repdst* matocsserv_repdst_jemalloc() {
	repdst *r;
	if (repdstfreehead) {
		r = repdstfreehead;
		repdstfreehead = r->next;
	} else {
		r = (repdst*)jemalloc(sizeof(repdst));
		passert(r);
	}
	return r;
}
...
//	if (FD_ISSET(lsock,rset)) {
		...
		if (ns<0) {
			...
			tcpnonblock(ns);
			tcpnodelay(ns);
			eptr = jemalloc(sizeof(matocsserventry));
			passert(eptr);
