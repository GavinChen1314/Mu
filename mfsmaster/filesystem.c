...
#define fsnode_dir_jemalloc() fsnode_jemalloc(0)
#define fsnode_file_jemalloc() fsnode_jemalloc(1)
#define fsnode_symlink_jemalloc() fsnode_jemalloc(2)
#define fsnode_dev_jemalloc() fsnode_jemalloc(3)
#define fsnode_other_jemalloc() fsnode_jemalloc(4)
...
static inline fsnode* fsnode_jemalloc(uint8_t indx) {
	fsnode_bucket *nrb;
	fsnode *ret;
...
#else
		nrb = (fsnode_bucket*)jemalloc(offsetof(fsnode_bucket)+nrbucketsize[indx]);
#endif
		passert(nrb);
...
static inline fsedge* fsedge_jemalloc(uint16_t nleng) {
	fsedge_bucket *erb;
	fsedge *ret;
...
#else
		erb = (fsedge_bucket*)jemalloc(offsetof(fsedge_bucket)+erbucketsize[indx]);
#endif
		passert(erb);
...
static inline uint8_t* symlink_jemalloc(uint16_t pathleng) {
	symlink_bucket *stb;
	uint8_t *ret;
...
#else
		stb = (symlink_bucket*)jemalloc(offsetof(symlink_bucket,bucket)+stbucketsize[indx]);
#endif
		passert(stb);
...
static inline uint64_t* chunktab_indx_jemalloc(uint8_t indx) {
	chunktab_bucket *ctb;
	uint64_t *ret;
...
#else
		ctb = (chunktab_bucket*)jemalloc(offsetof(chunktab_bucket,bucket)+ctbucketsize[indx]);
#endif
		passert(ctb);
...
static inline uint64_t* chunktab_realloc(uint64_t *oldchunktab,uint32_t oldchunks,uint32_t newchunks) {
	...
	if (oldindx==newindx) {
		...
		if (newindx>=CHUNKTAB_MAX_INDX) {
			...
			newchunktab = chunktab_indx_jemalloc(newindx);
...
void fsnodes_init_freebitmask (void) {
	...
	freebitmask = (uint32_t*)jemalloc(bitmasksize*sizeof(uint32_t));
	passert(freebitmask);
...
#else
		edgehashtab[i] = jemalloc(sizeof(fsedge*)*HASHTAB_LOSIZE);
#endif
...
#else
		nodehashtab[i] = jemalloc(sizeof(fsnode*)*HASHTAB_LOSIZE);
#endif
...
static inline void fsnodes_edgeid_init(void) {
	uint32_t h;
	edgeid_id_hashtab = jemalloc(sizeof(uint64_t)*EDGEID_HASHSIZE);
	edgeid_ptr_hashtab = jemalloc(sizeof(fsedge*)*EDGEID_HASHSIZE);
...
static inline uint8_t fsnodes_test_quota_for_uncommon_nodes(fsnode *dstnode,fsnode *srcnode,uint32_t inodes,uint64_t length,uint64_t size,uint64_t realsize) {
	...
	while (dstnode!=NULL) {
		if (dstnode->data.ddata.quota!=NULL) {
			nlptr = jemalloc(sizeof(struct _node_list));
			passert(nlptr);
			nlptr->node = dstnode;
			nlptr->next = dhead;
			dhead = nlptr;
...
static inline fsnode* fsnodes_create_node(uint32_t ts,fsnode* node,uint16_t nleng,const uint8_t *name,uint8_t type,uint16_t mode,uint16_t cumask,uint32_t uid,uint32_t gid,uint8_t copysgid) {
	...
	switch (type) {
		case TYPE_DIRECTORY:
			p = fsnode_dir_jemalloc();
			break;
		case TYPE_FILE:
		case TYPE_TRASH:
		case TYPE_SUSTAINED:
			p = fsnode_file_jemalloc();
			break;
		case TYPE_SYMLINK:
			p = fsnode_symlink_jemalloc();
			break;
		case TYPE_BLOCKDEV:
		case TYPE_CHARDEV:
			p = fsnode_dev_jemalloc();
			break;
		default:
			p = fsnode_other_jemalloc();
	}
	passert(p);
	nodes++;
...
static inline uint8_t fsnodes_append_slice_of_chunks(uint32_t ts,fsnode *dstobj,fsnode *srcobj,uint32_t slice_from,uint32_t slice_to) {
	...
	if (newchunks>dstobj->data.fdata.chunks) {
		if (dstobj->data.fdata.chunktab==NULL) {
			dstobj->data.fdata.chunktab = chunktab_jemalloc(newchunks);
		} else {
...
//				dstnode->mtime = srcnode->mtime;
				if (srcnode->data.fdata.chunks>0) {
					dstnode->data.fdata.chunktab = chunktab_jemalloc(srcnode->data.fdata.chunks);
			...
			if (srcnode->data.sdata.pleng>0) {
				dstnode->data.sdata.path = symlink_jemalloc(srcnode->data.sdata.pleng);
...
uint8_t fs_univ_symlink(uint32_t ts,uint32_t rootinode,uint8_t sesflags,uint32_t parent,uint16_t nleng,const uint8_t *name,uint32_t pleng,const uint8_t *path,uint32_t uid,uint32_t gids,uint32_t *gid,uint32_t auid,uint32_t agid,uint32_t *inode,uint8_t attr[ATTR_RECORD_SIZE]) {
	...
	*inode = 0;
	...
	newpath = symlink_jemalloc(pleng);
	passert(newpath);
...
//	}
	...
	if (indx>=p->data.fdata.chunks) {
		...
		if (p->data.fdata.chunktab==NULL) {
			p->data.fdata.chunktab = chunktab_jemalloc(newchunks);
		} else {
...
void fs_test_files() {
	...
	if (i==0) {
		...
		if (fsinfo_msgbuff==NULL) {
			fsinfo_msgbuff=jemalloc(MSGBUFFSIZE);
...
static inline int fs_loadnode(bio *fd,uint8_t mver,uint8_t *unodebuff) {
	...
	case TYPE_SYMLINK:
		...
		if (pleng>0) {
			if (pleng>MFS_SYMLINK_MAX) {
				...
				p->data.sdata.path = symlink_jemalloc(p->data.sdata.pleng);
				passert(p->data.sdata.path);
				memcpy(p->data.sdata.path,"... path too long ...",p->data.sdata.pleng);
				bio_skip(fd,pleng);
			} else {
				p->data.sdata.path = symlink_jemalloc(pleng);
				passert(p->data.sdata.path);
	...
	case TYPE_SUSTAINED:
		...
		if (ch>0) {
			p->data.fdata.chunktab = chunktab_jemalloc(ch);
			passert(p->data.fdata.chunktab);
		} else {
			p->data.fdata.chunktab = NULL;
		}
...
uint8_t fs_storenodes(bio *fd) {
	...
	auxbuff = jemalloc(1+4+1+1+1+2+4+4+4+4+4+2+8+4+2+8*65536+4*65536+4);
	for (i=0 ; i<noderehashpos ; i++) {
...
uint8_t fs_storeedges(bio *fd) {
	...
	auxbuff = jemalloc(4+4+8+2+65535);
...
int fs_importnodes(bio *fd,uint32_t mni) {
	...
	auxbuff = jemalloc(4+1+1+1+2+4+4+4+4+4+4+8+4+2+8*65536+4*65536+4);
