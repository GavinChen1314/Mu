...
uint8_t* matoclserv_createpacket(matoclserventry *eptr,uint32_t type,uint32_t size) {
	...
	psize = size+8;
	outpacket = jemalloc(offsetof(out_packetstruct,data)+psize);
...
//		locked = 1;
		i = CHUNKHASH(prevchunkid);
		lwc = jemalloc(sizeof(lwchunks)+sizeof(uint32_t)*gids);
		passert(lwc);
		lwc->chunkid = prevchunkid;
		lwc->fleng = fleng;
		lwc->eptr = eptr;
...
void matoclserv_mass_resolve_paths(matoclserventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if (length>tabsleng) {
		...
		inodetab = jemalloc(sizeof(uint32_t)*tabsleng);
		passert(inodetab);
		psizetab = jemalloc(sizeof(uint32_t)*tabsleng);
		passert(psizetab);
	}
	j = 0;
	totalsize = 0;
...
void matoclserv_fuse_sustained_inodes(matoclserventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if (length>inodetabsize) {
		...
		inodetab = jemalloc(sizeof(uint32_t)*inodetabsize);
		passert(inodetab);
	}
	rptr = data;
	j = 0;
...
void matoclserv_fuse_amtime_inodes(matoclserventry *eptr,const uint8_t *data,uint32_t length) {
	...
	if (length>tabsizes) {
		...
		inodetab = jemalloc(sizeof(uint32_t)*tabsizes);
		passert(inodetab);
		atimetab = jemalloc(sizeof(uint32_t)*tabsizes);
		passert(atimetab);
		mtimetab = jemalloc(sizeof(uint32_t)*tabsizes);
		passert(mtimetab);
	}
	rptr = data;
	j = 0;
...
uint32_t* matoclserv_gid_storage(uint32_t gids) {
	...
	if (gids==0) {
		...
		if (gidleng<gids) {
			...
			gid = jemalloc(sizeof(uint32_t)*gidleng);
			passert(gid);
		}
		return gid;
	}
...
void matoclserv_read(matoclserventry *eptr,double now) {
	...
	if (readbuffsize==0) {
		readbuffsize = 65536;
		readbuff = jemalloc(readbuffsize);
		passert(readbuff);
	}
	...
	while (rbpos<rbleng) {
		...
		if (eptr->input_packet == NULL) {
			...
			stats_prcvd++;
			eptr->input_packet = jemalloc(offsetof(in_packetstruct,data)+leng);
...
//	if (FD_ISSET(lsock,rset)) {
		...
		if (ns<0) {
			...
			tcpnonblock(ns);
			tcpnodelay(ns);
			eptr = jemalloc(sizeof(matoclserventry));
			passert(eptr);
