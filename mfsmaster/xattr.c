...
uint8_t xattr_copy(uint32_t srcinode,uint32_t dstinode) {
	...
	dxe = jemalloc(sizeof(xattrentry));
	passert(dxe);
	dxe->inode = dstinode;
	dxe->pairhead = NULL;
	xpt = &(dxe->pairhead);
...
int xattr_load(bio *fd,uint8_t mver,int ignoreflag) {
	...
	databuff = jemalloc(MFS_XATTR_NAME_MAX+MFS_XATTR_SIZE_MAX);
	passert(databuff);
	lastinode = 0;
	while (1) {
